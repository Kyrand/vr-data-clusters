<script>
	import { onMount } from 'svelte'
	import AframeAssets from './AFrameAssets.svelte'

	const { environment, stats, children, background = 'color: black' } = $props()

	let AFRAME = null
	let loaded = $state(false)
	onMount(async () => {
		console.log('Mounting VR central...')
		AFRAME = (await import('aframe')).default
		// await import('aframe-orbit-controls')
		try {
			await import('@lib/aframe/AF_components.js')
			console.log('Loaded AFRAME components...')
		} catch (error) {
			console.log(error)
		}
		loaded = true
	})
</script>

{#if loaded}
	<a-scene
		{environment}
		{background}
		{stats}
		cursor="rayOrigin: mouse"
		raycaster="objects: [html];"
	>
		<!-- <a-scene {background} {stats}> -->
		<a-entity
			camera
			look-controls="enabled: false"
			orbit-controls="target: 0 0 0; minDistance: 1; maxDistance: 180; initialPosition: 0 0 1.25; rotateSpeed: 0.5"
		/>
		<AframeAssets />
		{@render children()}
	</a-scene>
{/if}

<style>
	.ui {
		position: absolute;
		z-index: 99;
	}
</style>
