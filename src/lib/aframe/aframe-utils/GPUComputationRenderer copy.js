...
export default function GPUComputationRenderer(sizeX, sizeY, renderer) {
	...
	this.init = function () {
		if (!renderer.capabilities.isWebGL2 && !renderer.extensions.get('OES_texture_float')) {
			return 'No OES_texture_float support for float textures. (WebGL 1)'
		}
	...
	function addResolutionDefine(materialShader) {
		console.log(`Setting resolution to [${sizeX.toFixed(1)},${sizeY.toFixed(1)}]`)
	...
	this.doRenderTarget = function (material, output) {
		...
		let vrFlag = renderer.xr.enabled
		renderer.xr.enabled = false
		renderer.render(scene, camera, output)
		renderer.xr.enabled = vrFlag
	...
	this.testFloatRenderTarget = function () {
		...
		var renderTarget = this.createRenderTarget(
			16,
			16,
			undefined,
			undefined,
