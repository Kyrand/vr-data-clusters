...
export function createScaleGrid(scaleW, scaleH, color = 0xcccccc, gridNum = 11, opacity = 0.2) {
	let grids
	...
	// width
	if (scaleW.__type == consts.FIELD_TYPES.Categorical) {
		grids = scaleW.domain()
	} else {
	...
	const vertices = []
	grids.forEach((g) => {
		vertices.push(new THREE.Vector3(scaleW(g), scaleH.range()[0], 0))
		vertices.push(new THREE.Vector3(scaleW(g), scaleH.range()[1], 0))
	})

	// height
	if (scaleH.__type == consts.FIELD_TYPES.Categorical) {
		grids = scaleH.domain()
	} else {
	...
	grids.forEach((g) => {
		vertices.push(new THREE.Vector3(scaleW.range()[0], scaleH(g), 0))
		vertices.push(new THREE.Vector3(scaleW.range()[1], scaleH(g), 0))
	})

	gridGeo.setFromPoints(vertices)
	let line = new THREE.LineSegments(gridGeo, material, THREE.LinePieces)
	gridObject.add(line)
