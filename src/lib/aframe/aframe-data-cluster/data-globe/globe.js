...
export default function (globeEl) {
	...
	let world = {
		...
		initD3: function (opts) {
			...
			this.borders = topojson.mesh(opts.data, opts.data.objects.countries, function (a, b) {
				return a !== b
			})
		...
		genMesh: function (opts) {
			...
			let mesh = new THREE.Mesh(
				new THREE.SphereGeometry(this.sphereRadius, segments, segments),
				material
			)
		...
		genTexture: function (opts) {
			...
			let path = d3.geoPath().projection(this.projection).context(ctx)
			...
			if (opts.land) {
				...
				path(opts.borders)
				ctx.stroke()
				ctx.strokeStyle = this.gratiColor
				ctx.lineWidth = 0.25
				ctx.beginPath()
				path(graticule())
				ctx.stroke()
