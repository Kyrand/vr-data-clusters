...
export function renderHexJSON(hexjson, width, height) {
	...
	var qmax = max(hexes, function (d) {
			return +d.q
		}),
		qmin = min(hexes, function (d) {
			return +d.q
		}),
		rmax = max(hexes, function (d) {
			return +d.r
		}),
		rmin = min(hexes, function (d) {
			return +d.r
		})
	...
	if (layout === 'odd-r' || layout === 'even-r') {
		hexRadius = min([width / ((qnum + 0.5) * Math.sqrt(3)), height / ((rnum + 1 / 3) * 1.5)])
	} else {
		hexRadius = min([height / ((rnum + 0.5) * Math.sqrt(3)), width / ((qnum + 1 / 3) * 1.5)])
	}
			...
			for (var j = i + 1; j < maxHex; j++) {
				...
				if (hex[field] !== otherHex[field]) {
					if (Math.abs(hex.q - otherHex.q) <= 1 && Math.abs(hex.r - otherHex.r) <= 1) {
						if (
							(hex.x - otherHex.x) * (hex.x - otherHex.x) +
								(hex.y - otherHex.y) * (hex.y - otherHex.y) <
							hexRadiusSquared
						) {
							// They're neighbours
							var midpoint = {}
