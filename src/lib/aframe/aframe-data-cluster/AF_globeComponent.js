...
AFRAME.registerComponent('globe', {
	...
	init: function () {
		...
		this.el.addEventListener(
			...
			() =>
				state.hoverObj &&
				this.data.onClick &&
				this.data.onClick(formatObjForInteraction(state.hoverObj), state.hoverEvent)
		)
	},
	...
	tick: function (t, td) {
		...
		if (props.onHover || props.onClick) {
			...
			while (topObject && !topObject.hasOwnProperty('__globeObjType')) topObject = topObject.parent
		...
		if (topObject !== state.hoverObj) {
			props.onHover &&
				props.onHover(formatObjForInteraction(topObject), formatObjForInteraction(state.hoverObj))
