...
export function createAxesLabels(dataCluster) {
  ...
  let axesLabels = d3.select(dataCluster.el)
      .append("a-entity")
...
export function createGroupLabels(dataCluster) {

  let groupLabels = d3.select(dataCluster.el)
      .select(".group-labels")
