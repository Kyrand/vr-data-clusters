...
let randomizePosition = function() {
  d3.select(this)
    .attr("animation", `property: position; dur: 1000; to: ${randomPos()}`)
}
...
AFRAME.registerComponent("data-cluster", {
  ...
  initCluster: function(clusterData) {
    ...
    clusters.attr("geometry", {type: "box", params: {height: clusterWidth, width: clusterWidth, depth: clusterWidth}})
      .attr("material", `side: double; color: #00ff00; opacity: ${cfg.clusterMaterialOpacity}; transparent: true`)
    // .on("hit", (e) => {
    ...
    this.dataPoints = clusters.selectAll("a-entity")
  ...
  clusterXYZ: function() {
    this.dataPoints
      .attr("animation", d => `property: position; dur: 1000; to: ${this.getDatapointPos(d)}`)
  },

  animate: function() {
    let t = d3.interval(() => {
      clusters.selectAll("a-entity")
        .each(randomizePosition)
      //t.stop()
    }, cfg.animationTime)
  },
